 ...
 */
export type OfHandlerType<T extends MiddlewareHandler> = T extends MiddlewareHandler<infer E, infer P, infer I>
  ? {
      env: E;
      path: P;
      input: I;
...
export type RouteHook<
  ...
  I extends Input = InputTypeParam<R> &
    ...
    InputTypeJson<R>,
  P extends string = ConvertPathType<R['path']>,
> = Hook<I, E, P, RouteConfigToTypedResponse<R> | Response | Promise<Response> | void | Promise<void>>;
...
export class OpenAPIHono<E extends Env = Env, S extends Schema = {}, BasePath extends string = '/'> extends Hono<
  E,
  S,
  BasePath
> {
  ...
  >(
    ...
    handler: Handler<
      ...
      R['middleware'] extends MiddlewareHandler[] | MiddlewareHandler ? RouteMiddlewareParams<R>['env'] & E : E,
      P,
      I,
      // If response type is defined, only TypedResponse is allowed.
      R extends {
  ...
  ): OpenAPIHono<E, S & ToSchema<R['method'], MergePath<BasePath, P>, I, RouteConfigToTypedResponse<R>>, BasePath> => {
  ...
  getOpenAPIDocument = (config: OpenAPIObjectConfig): ReturnType<typeof generator.generateDocument> => {
  ...
  getOpenAPI31Document = (config: OpenAPIObjectConfig): ReturnType<typeof generator.generateDocument> => {
...
export const createRoute = <P extends string, R extends Omit<RouteConfig, 'path'> & { path: P }>(routeConfig: R) => {
  const route = {
    ...routeConfig,
